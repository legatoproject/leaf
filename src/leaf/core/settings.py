...
class Setting():
    ...
    def __init__(self, key: str,
                 default: str = None,
                 validator: callable = DefaultValidator()):
        self.__key = key
        self.__validator = validator
        self.__default = default
        if default is not None and not self.is_valid(default):
            raise ValueError(
                "Invalid default value {}".format(default))
    ...
    def value(self, newvalue: str):
        if newvalue is None:
            ...
            if not self.is_valid(newvalue):
                raise ValueError("Invalid value: {}".format(newvalue))
    ...
    def as_boolean(self):
        ...
        if v is None:
            return False
        return v.strip().lower() not in Setting.__FALSE_VALUES
