...
def init_common_args(parser, add_rm_env=False, env_scripts=False, add_rm_packages=False, with_scope=False):
    if add_rm_env:
        ...
        parser.add_argument("--unset", dest="env_rm_list", action="append", metavar="KEY", help="remote given environment variable")
    if env_scripts:
    ...
    if add_rm_packages:
        parser.add_argument("-p", "--add-package", dest="pkg_add_list", action="append", metavar="PKGNAME", help="add given package")
